...
## Round 12 — "500 error on login" traced to the Feedback module's error handling, not BTP (2026-09-15)

User reported a 500 error immediately after logging in as a student. Not a BTP bug — Student accounts land on `/academics/feedback/student` by default after login (a different module entirely), and `dev.student` was only ever set up for BTP testing, so it has zero course enrollments for Feedback. The backend correctly 404s (`"Student is not enrolled in any courses for this semester"`) — but `feedbackDashboard.jsx`'s loader had a real bug: it unconditionally threw a hardcoded `status: 500` for **any** non-OK response, discarding both the real status code and the backend's actual message (there was even a leftover `//add custom messages for 403 and 404` TODO comment confirming this was known-incomplete). So a legitimate, specific 404 was displayed to the user as a generic, alarming "500 Error loading Feedback dashboard."

**Fixed**: the loader now throws with the real `response.status` and the backend's actual `message` instead of hardcoding both. Verified live: the same request now correctly shows a 404 (matches `ErrorPage.jsx`'s generic "page not found" text for any 404, which is a pre-existing, unrelated design choice for navigation errors generally — not changed here) instead of a false 500.

**Does not affect BTP/Honors/AP testing** — `dev.student`'s BTP data and every other BTP/Honors/AP endpoint are untouched by this; the fix is scoped to the Feedback module's error handling only. To skip this page during testing, navigate directly to `/academics/btp/student` (or `/academics/honors/student`, `/academics/ap/student`) instead of letting the default post-login redirect land on Feedback first.

## Round 13 — AP student's Completed page said "BTP Project Completed"; mislabeling bug, fixed everywhere it occurred (2026-09-15)

User noticed the AP student's completed-project page displayed "BTP" instead of "Additional Project" — correctly pointed out that BTP and AP are independent, so this made no sense. Root cause: `Completed.jsx` (shared by BTP/Honors/AP student views) had `const label = programType === "honors" ? "Honors" : "BTP"` — a ternary written back when only BTP and Honors existed, never updated when AP was added, so anything that wasn't literally `"honors"` fell through to `"BTP"`. The underlying project *data* was correct the whole time (real AP project name/description, not BTP's) — only the heading label was wrong, which is exactly why it looked like "the wrong project" rather than "a mislabeled one."

Fixed by reading the label from the same shared `programConfig.js` every other component already uses (`getConfig(programType).label` → "BTP"/"Honors"/"Additional Project") instead of a hand-rolled ternary — the config already had the right value, this component just wasn't using it. Also cleaned up the resulting "Additional Project Project Completed" double-wording (dropped the template's own hardcoded "Project").

**Found and fixed the identical defect in two more places while checking for it** (same hardcoded `"BTP"` heading, no ternary at all this time — always literally "BTP" regardless of `programType`, which the component already received as a prop): the student's in-progress view (`Inprogress.jsx`) and the guide's project view (`StudentInProgress.jsx`) — both previously flagged in this doc as a known-not-fixed cosmetic issue, now fixed using the same `config.label` pattern.

**Verified live**: `dev.student`'s completed AP project ("AP Test Project - Evaluator Verification") now correctly shows "Additional Project Completed" instead of "BTP Project Completed".

## Round 15 — configurable evaluations-per-semester with per-evaluation max marks, BTP/Honors/AP (2026-09-15)

User asked for two related features: (1) let the guide choose how many evaluations happen per semester (common default 2, but some guides want more), and (2) let the guide set a different max-marks weight per evaluation (e.g. 3 evals in a semester weighted 25/25/50) — with plain "Evaluation 1/2/3" labeling throughout, not "Midsem/Endsem" (an earlier idea from this conversation, dropped once the marks-weighting requirement made a binary midsem/endsem label not generalize).

**Design**: semesters-per-program stay fixed (BTP 2, Honors 4, AP 1 — tied to the degree structure), but the evaluation schedule *within* a semester becomes a per-project `evaluationConfig: [{maxMarks}]` array, set once by the guide **when they approve a request** (not at topic-posting time, since AP has no pre-existing topic to attach it to — proposals are student-initiated). The same array repeats identically every semester of the project. Total evaluations for a project = `evaluationConfig.length × semesters`. Defaults to `[{maxMarks:50},{maxMarks:50}]` (2/semester) if the guide doesn't customize it, matching prior behavior.

**Backend** (mirrored ×3 as always):
- `BTP.js`/`Honors.js`/`AP.js`: new `evaluationConfig` field on the project schema.
- `BTPEvaluation.js`/`HonorsEvaluation.js`/`APEvaluation.js`: new `maxMarks` field, copied onto each evaluation round from the project's config at creation time (so a round keeps its weight even though the config array itself never changes after project creation).
- `approveTopicRequest`/`approveHonorsTopicRequest`/`approveAPRequest`: accept an optional `evaluationConfig` in the request body, validated (non-empty array, every entry a positive `maxMarks`), defaulting to 2×50 if omitted.
- `evaluateProjectasGuide`/`evaluateHonorsProjectasGuide`/`evaluateAPProjectasGuide`: the old hardcoded `BTP_MAX_EVALUATIONS`/`HONORS_MAX_EVALUATIONS`/`AP_MAX_EVALUATIONS` constants are gone — max evaluations and the current round's cap are now computed from `project.evaluationConfig.length × <fixed semester count>`, cycling through the config array per semester (round index `% evaluationConfig.length`). Guide-submitted marks are now validated server-side against that round's cap (400 if exceeded).
- `evaluateProjectasEval`/Honors/AP equivalents: panel evaluator marks validated against `evaluation.maxMarks` the same way.
- `viewProject`/`viewProjectEvaluator` and both Honors/AP equivalents, plus all three `ugstudent*Controller.js` dashboards: these all manually project a `project`/`evaluations` object field-by-field rather than returning the Mongoose document directly — `evaluationConfig` (on the project) and `maxMarks` (on each evaluation) had to be added explicitly to every one of these hand-built response shapes, or the frontend would never see them despite the schema having the data.

**Frontend**:
- `Requests.jsx` (BTP/Honors shared) and `APRequests.jsx`: the "More info" expanded request card now has an evaluation-config editor (add/remove rows, each a marks input) before Accept/Reject, defaulting to 2×50; submitted as part of the approve payload.
- `EvaluatorModal.jsx`: the "Enter marks: /50" hint is no longer hardcoded — takes a `maxMarks` prop and shows the real cap, applied as the input's `max` attribute too.
- That prop is threaded from two different places depending on who's evaluating: `StudentInProgress.jsx` (guide, new round) computes it from `project.evaluationConfig[evalCount % length]` since the round doesn't exist yet; `ViewProjEvaluator.jsx` (panel evaluator, existing pending round) just reads `pendingEval.maxMarks` directly and passes it down through `UpdateList.jsx` → `Buttons.jsx`.
- `ReleaseEvaluationPanel.jsx` and `PastEvaluationsPanel.jsx`: every mark shown (guide's, each evaluator's, the final pooled grade) now displays as "X/maxMarks" instead of a bare number.
- `Inprogress.jsx` (student's own view) "Latest Marks" now shows "X/maxMarks" too.
- **Found and fixed a bug this change would otherwise have made worse**: `StudentInProgress.jsx`, `ViewProjEvaluator.jsx`, and `Inprogress.jsx` all computed "Evaluations Completed X of Y" and the semester/progress-pill display from the *static* `programConfig.js` totals (`config.totalEvals`) — already wrong the moment a project's `evaluationConfig` differs from the default. All three now derive the real total from `evaluationConfig.length × config.semesters`, falling back to the static default only for older projects with no config. `getSemesterLabel()` in `programConfig.js` gained an optional `evalsPerSemOverride` parameter for the same reason.

**Verified end-to-end via a real browser click** (not just curl, to rule out both a backend-logic bug and a stuck frontend wiring bug at once): created a disposable BTP student, requested a topic, opened **Requests → More info**, set eval 1 and 2 to 25 marks each and added a 3rd row defaulting to 50 (reproducing the user's own example), clicked **Accept** — confirmed via direct DB query that `evaluationConfig: [25, 25, 50]` was saved exactly as entered. Reloaded the new project's page as guide: "Evaluations Completed 0 of **6**" (3 evals/semester × 2 semesters — previously would have shown the stale hardcoded "0 of 4"). Clicked **Evaluate now**: the marks input correctly showed "**/25**" for round 1, not the old hardcoded "/50". Backend validation separately curl-tested across all 4 rounds of a 3-slot config: 30 marks against a 25 cap correctly rejected (400), 22/25/45/20 correctly accepted, and round 4's cap correctly cycled back to 25 (confirming the semester-repeat behavior, not just the first pass through the array).

**Not yet done**: the "final mark" field in the guide's pooling panel (`ReleaseEvaluationPanel.jsx`) has a client-side `max` hint but the backend's `releaseEvaluation` doesn't enforce the cap on that value — a deliberate, minimal-scope choice (pooling may reasonably want to round or otherwise diverge slightly from either raw input), not an oversight, but flagging it here in case that assumption turns out to be wrong.

## Round 16 — wrong frontend was serving port 5173, masking Round 15's UI entirely (2026-09-16)

User reported seeing only "Accept"/"Reject" with no evaluation-config editor, then separately noticed no Honors/AP UI at all. Root cause: the process on port 5173 was `Institute_Portal/frontend` (the abandoned pre-pivot frontend from Round 3) — **not** `feedback_frontend`. Confirmed by diffing the served `/src/App.jsx` against both repos: zero matches for `honors`/`APDashboard`/`APFacultyRouter`, meaning the live app had no Honors or AP routing at all, which only `Institute_Portal/frontend` lacks. Both apps share an identical `index.html` (title "Feedback Form", same boilerplate) so there was no visual tell from the page chrome alone that the wrong one was running.

**Why this happened**: `Institute_Portal/frontend`'s `vite.config.js` never set an explicit dev port, so it defaults to Vite's standard 5173 — the exact port `feedback_frontend` needs (and the only origin the backend's CORS allowlist accepts locally). A background task tracked from earlier in this session (`"Start frontend dev server"`) had apparently been running that old frontend the whole time; something restarted it before `feedback_frontend`'s own dev server could claim the port today, and it silently won.

**Fixed two ways**: (1) killed the wrong process and started `feedback_frontend`'s dev server fresh on 5173, confirmed via the same `/src/App.jsx` content check that Honors/AP routing is present again. (2) Pinned `Institute_Portal/frontend/vite.config.js` to `server: { port: 5199, strictPort: true }` — structural fix so this specific collision can't recur even if that stale background task (or anyone else) starts that dev server again; it'll bind to 5199 or fail outright instead of silently masquerading as the real app on 5173.

**If this happens again**: check `netstat -ano | findstr :5173` for the PID, then `Get-CimInstance Win32_Process -Filter "ProcessId=<pid>"` — if `CommandLine` resolves through `Institute_Portal\frontend\node_modules`, it's the wrong one.

## Round 17 — "Stop Guiding" button, for when a student leaves BTP/Honors/AP mid-project (2026-09-16)

User asked for a way to end a guide's relationship with a project before completion (e.g. the student drops out). Built and mirrored across all three programs, matching the same pattern as Round 10's Remove Evaluator.

- New `status` value `"discontinued"` on the `BTP`/`Honors`/`AP` schemas — distinct from `"completed"` so it's never mistaken for a finished project, and the project record (plus any evaluations already given) is preserved rather than deleted.
- New `stopGuiding` controller (mirrored ×3), guide-only, only on `active` projects: flips the project to `discontinued` and clears `project: null` on the student's `BTPRegistration`/`HonorsRegistration`/`APRegistration` — the same field `approveTopicRequest` checks to refuse a student who's "already in a project", so this is exactly what frees them to request a new topic or join a different program. New `POST /faculty/{btp,honors,ap}/stopguiding` route each.
- `StudentInProgress.jsx` (the guide's shared project page): new **"Stop Guiding"** button next to the project title, gated behind `window.confirm` (a genuinely destructive, hard-to-reverse-from-the-UI action) and hidden once a project is already completed. On success, navigates the guide back to their dashboard rather than staying on a page for a project that's no longer active.

**Verified end-to-end via curl** (not a live click — the button correctly triggers a native `window.confirm()` dialog, which browser-automation safety rules say not to trigger programmatically, since it can hang the extension; visually confirmed the button renders in the right place first): called `stopguiding` on the disposable smoketest BTP project → project status flipped to `discontinued` → the student's `BTPRegistration.project` confirmed `null` in the DB → logged in as that student and confirmed their dashboard phase is back to `TOPIC_SELECTION` (the real topic-browsing view), not stuck showing a stale in-progress project.

**Follow-up, same round**: user asked for the native `window.confirm`/`alert` in Stop Guiding to become a proper toast UI. Replaced with the same libraries already used elsewhere in this app for exactly this (`sweetalert2` for the confirm step, `react-toastify` for the result message) rather than inventing a new pattern - `AdminDashboard.jsx`'s Stop/Reset Feedback buttons already do this. `Swal.fire` renders a DOM modal (not a blocking native dialog), so it's safe to drive with browser automation, unlike `window.confirm`. Verified live end-to-end with a real click: confirm dialog shows the right title/text, clicking through correctly calls the backend, navigates back to the dashboard, and the DB shows `discontinued` + freed registration - not just curl this time.

## Round 18 — student could only see their most recent mark, not their full history; a real bug, not just a missing feature (2026-09-16)

User asked to let BTP (and Honors/AP) students see every mark they've received, not just the latest. While building it, found that the existing "Remark" field had an actual bug, not just missing scope: `Inprogress.jsx` computed it via `evaluations.find(e => e.time && e.remark)` — since the backend always requires a remark and `evaluations` is returned oldest-first, `.find()`'s first match is always `evaluations[0]`, i.e. the **oldest** round's remark, permanently, no matter how many rounds followed. "Latest Marks" right next to it was already correct (it used `.reverse().find(...)`), so the two labels had been silently showing mismatched rounds this whole time - marks from the latest evaluation, remark from the first one ever given.

**Fixed both together**, since fixing "show all marks" naturally required correctly identifying "latest" first: `Inprogress.jsx` (shared by all three programs' student routers - one fix covers BTP/Honors/AP) now filters `evaluations` down to only the ones actually released (`marksgiven` present - unreleased rounds come back from the backend with `marksgiven: null`), and takes the *last* entry in that filtered, oldest-first list as "latest" - fixing the remark bug as a side effect. Added a **"View all marks (N)"** toggle beneath Latest Marks/Remark that expands into every released round's date, remark, and mark (as `X/maxMarks`, matching Round 15's per-round weighting) - reusing `ReleaseEvaluationPanel.module.css`'s existing classes rather than new CSS, same as `PastEvaluationsPanel.jsx` does on the guide side.

**Verified live**: `dev.honorsstudent`'s dashboard now correctly shows "Latest Marks: 41/50" with the matching latest remark ("Semester 2 guide evaluation..."), and clicking "View all marks (2)" correctly lists both released rounds - Round 1: 45/50 with its own remark, Round 2: 41/50 with its own - each showing its actual own data, not a mismatched pairing.

## Round 14 — BTP topic requests were silently misrouted to the AP backend; core request flow broken, fixed (2026-09-15)

User (as a fresh `dev.btpstudent2` account) requested the BTP topic and got a genuine "Oops! Unexpected token '<', "<!DOCTYPE "... is not valid JSON" crash — not a stale-session false alarm this time. Reproduced live: the request went to `POST /student/ap/requesttopic` (404, returns Vite's HTML error page, which the frontend then tried to `.json()`-parse) instead of `/student/btp/requesttopic`.

**Root cause**: `programConfig.js`'s `detectProgramType(url)` used a bare `path.includes("/ap")` substring check. The BTP request-topic action's URL is `/academics/btp/student/applytotopic` — which contains the literal substring `/ap` inside the word "**ap**plytotopic". So every BTP `applytotopic` submission was silently misdetected as `programType: "ap"` and routed to the wrong backend base, 404ing every time. The same substring bug also affects `approvetopicrequest` (`/approvetopicrequest` also starts with "ap" right after the slash) — meaning the **guide's own "Accept" action for BTP topic requests has the identical bug**, not yet independently reproduced but certain to fail the same way once a BTP request actually reaches the guide's approve click, since it goes through the same `detectProgramType(request.url)` call in `Requests.jsx`.

Honors was accidentally safe from this specific collision (its own segment check, `path.includes("/honors")`, is evaluated first and matches before the `/ap` check is ever reached), which is why this went unnoticed through all of Honors' extensive testing this session but never actually got exercised for BTP's request/approve cycle specifically.

**Fixed**: `detectProgramType` now matches `/honors` and `/ap` as whole path segments via regex (`/\/ap(\/|$)/` — requires `/ap` to be followed by `/` or end-of-string), not a raw substring, so `/applytotopic` and `/approvetopicrequest` no longer false-positive. Verified live: `dev.btpstudent2`'s request now correctly hits `/student/btp/requesttopic`, returns 200, and the DB shows a real `BTPRegistration` with a pending request against the correct topic — confirmed via direct query, not just the UI's optimistic "Already Requested" label.

**Not yet separately re-verified**: the guide's "Accept" click on this exact pending request (should now also route correctly to `/faculty/btp/approvetopicrequest` given the same fix, but wasn't re-tested end-to-end after the fix landed — next thing to check when continuing the BTP student walkthrough).

## Round 19 — Meeting Scheduler feature: faculty availability, student booking requests, race-safe (2026-09-16)

User asked for a faculty↔student meeting scheduler: faculty post available time slots, students request to book one, faculty approve/reject. Built end-to-end, new feature area — no existing "slot"/"availability"/"booking" concept existed anywhere in the backend or either frontend before this.

**Backend** (shared, used by both frontends): new `backend/models/MeetingSlot.js` — single-collection state machine `available → requested → booked`, with `available|booked → cancelled`, embedding the request (`requestedBy`, `purpose`, `requestedAt`) directly on the slot rather than a separate request collection (unlike AP's `APRegistration`/`APFacultyRequest` split), since a slot can have at most one active request at a time — that's the entire double-booking constraint, and it lets the race-safe transition be a single atomic `findOneAndUpdate`. New `backend/controllers/{faculty,student}meetingController.js` + `backend/routes/{faculty,student}meetingRoutes.js`, mounted at `/faculty/meetings` and `/student/meetings`, mirroring the AP module's controller/route split and middleware conventions exactly. New `backend/utils/slotTime.js` (`getSlotStart`/`isSlotInPast`) shared by both controllers for the past-time guard.

**Race-condition safety (the core requirement — "can't go wrong")**: every state transition is a `findOneAndUpdate({_id, status: <required current state>, ...})` — never a read-then-write. Verified live by firing two concurrent `POST /student/meetings/request` calls at the same slot: exactly one returned 200, the other 409 "This slot is no longer available", DB left with exactly one `requestedBy`. Also verified: approving/rejecting/cancelling a slot twice in a row correctly 409s the second time; requesting or creating a past-dated slot correctly 400s.

**Frontend — built twice, second build is the one that matters**: initially built the full UI in `Institute_Portal/frontend`, tested it working end-to-end there, then discovered (re-reading this file's own Round 3) that `Institute_Portal/frontend` is the abandoned pre-pivot frontend — `feedback_frontend` is the real, deployed one. Ported the whole feature to `feedback_frontend` instead, adapting to its actual conventions (confirmed by direct comparison against its AP module, the closest existing analog): all mutations use `useSubmit()` + a route `action` that always ends in `redirect()` — no `useFetcher` exists anywhere in that repo and none was introduced; `toast.error`/`toast.success` are called directly inside the action (before the redirect) for both the happy path and recoverable failures like the 409 race-loss, so a losing request gets friendly feedback instead of crashing to `ErrorPage` — a deliberate, small deviation from AP's own silent-redirect-only convention, justified specifically by this feature's race-condition requirement. Cancelling a booked slot confirms via `Swal.fire` (matches the repo's established destructive-action pattern, e.g. BTP's "Stop Guiding"), not the ad-hoc dialog component built during the first, discarded pass. New files: `src/pages/Meetings{Router,FacultyRouter,StudentRouter}.jsx`, `src/components/Academics/Meetings/{faculty,student}/*.jsx`, `src/components/styles/Meetings.module.css`; new `academics/meetings` route subtree in `App.jsx` (faculty: single dashboard; student: nested `index`/`:facultyId/slots`/`mybookings` routes with a tab nav) mirroring the AP block exactly; new "Meetings" sidebar entry using the repo's own already-present, previously-unused `calendar.png` asset. Left the first, now-dead copy in `Institute_Portal/frontend` in place (harmless, matches how that repo already accumulates abandoned duplicate implementations) rather than spending more time deleting it.

**Verified live end-to-end in `feedback_frontend` on port 5173** (the real app, not just curl): faculty creates a slot → student sees it, requests it with a purpose → faculty's Pending Requests shows it → Approve → slot flips to Booked, visible on both sides → Cancel on a booked slot triggers the `Swal.fire` confirmation → student's My Meetings still shows the cancelled slot (not silently gone). `npx vite build` clean; `npx eslint` on the new files shows only the same two pre-existing-pattern warning classes (`react-refresh/only-export-components`, `no-case-declarations`) that AP's own files trigger identically — confirmed by linting `APFacultyRouter.jsx`/`APRequests.jsx` for comparison, not a new problem introduced.

**Dev environment note for next time**: `dev.faculty@iiits.in`'s password changed on its own mid-session (worked via curl, then a `curl`/browser login minutes later failed with the same password, twice) — re-ran `_createdevfaculty.mjs --reset` each time as a workaround, never root-caused. Worth checking for a concurrent process/session touching the same dev account if it recurs.

## Round 20 — AP live-testing surfaced a real bug: auto-completing a project on the final evaluation cut the student off from their own marks (2026-09-16)

Live-tested AP for the first time this session (Round 11 had left it unverified). Immediately hit it: as soon as the guide submitted the second (final, per default `evaluationConfig`) evaluation, the student's dashboard stopped showing progress/marks entirely and instead rendered a bare "Project Completed" page with no numbers on it at all - not a display bug, a genuine access-loss bug.

**Root cause**: `evaluateAPProjectasGuide` (and its BTP/Honors mirrors) auto-set `project.status = "completed"` the instant the evaluation count reached `evaluationConfig.length * <semesters>` - immediately, even when the project still had unpooled/unreleased evaluator marks (`canstudentsee: false`). `getAPDashboard` (and its BTP/Honors mirrors) then special-cased `status === "completed"` into an abbreviated response - `{_id, name, about, status}` only, no `evaluations`, `team`, `guide`, or `evaluationConfig` - so the instant a project auto-completed, the student's own marks/remarks/history became permanently unreachable through the UI, with no way back in.

**User's requested fix, built as asked**: completion is now an **explicit guide action**, not automatic.
- Backend: removed both auto-complete side effects from `evaluate{BTP,Honors,AP}ProjectasGuide` (blocking the eval past the max still 400s, it just no longer flips `status` itself). Added a new `completeProject` endpoint per program (`POST /faculty/{btp,honors,ap}/completeproject`) that guide-authenticates, then requires all configured evaluations to exist *and* all of them to be released (`canstudentsee: true` on every one) before setting `status = "completed"` - so a guide can't mark complete while marks are still sitting unpooled from evaluators.
- `getBTPDashboard`/`getHonorsDashboard`/`getAPDashboard` no longer special-case completed projects into a stripped response - they now always return the full `evaluations`/`team`/`guide`/`evaluationConfig` payload, with `phase` set to `"COMPLETED"` vs `"IN_PROGRESS"` and an informational `message` as the only things that change based on status.
- Frontend: `Inprogress.jsx` (shared by all three programs' student routers) now renders a green completion banner at the top when `project.status === "completed"`, while everything below it - progress bar, latest marks, "View marks history" - renders exactly as it does mid-project, just with the "Add update" form hidden. `BTPStudentRouter.jsx`/`HonorsStudentRouter.jsx`/`APStudentRouter.jsx` route the `COMPLETED` phase to `Inprogress` now instead of the old dead-end `Completed.jsx`, which had zero remaining references after this change and was deleted. `StudentInProgress.jsx` (guide side) hides "Evaluate now" once the configured evaluation count is reached, and shows a new **"Mark {label} Complete"** button (next to "Stop Guiding") once every round is both evaluated *and* released - confirmed via `Swal.fire`, result reported via `toast`, matching the `handleStopGuiding` pattern from Round 17 exactly (no native dialogs, per that round's standing instruction).

**Verified live, two ways**: (1) two pre-existing AP projects that had already hit the old auto-complete bug during earlier testing - confirmed the student dashboard now shows full history (`dev.apstudent`: "Latest Marks 45/50", "View marks history" expanding to both rounds with their own remarks) instead of the dead-end page, with zero data migration needed since the fix is purely about what the dashboard returns for an already-`completed` project. (2) a brand-new AP project end-to-end with a throwaway `dev.apstudent4` account: proposed → accepted with default `evaluationConfig` → evaluated round 1 (status stayed `active`, no button yet) → evaluated round 2 (status **still `active`** - the auto-complete regression check) → a 3rd evaluation attempt correctly 400'd ("Mark the project complete instead of evaluating again") → `completeproject` correctly flipped status to `completed` → student dashboard immediately showed the green banner plus "Evaluations Completed 2 of 2", "Latest Marks 47/50", and both rounds under "View marks history".

**Process note**: lost time mid-round to a backend restart that silently kept serving stale code - `taskkill` reported "process not found" for a PID that turned out to be wrong, then a new `node index.js` bound port 3000 successfully and looked correctly started (logged "Server Running on port 3000" / "DB connected"), but requests were still hitting the *old* process the whole time. Only caught it by planting a throwaway marker field in a response and confirming it was absent. Root cause not fully pinned down; going forward, verify a backend restart landed by checking `netstat` for the listening PID *and* confirming that PID's own `Get-Process -Id <pid> | Select StartTime` is after the edit, not just trusting the startup log or a `taskkill` exit message.